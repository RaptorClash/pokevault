...
class Gen1Strategy extends CatchRateStrategy {
  ...
  CatchRateResult calculate(CatchRateParams params) {
    ...
    if (params.ballId == 'great') glitchWarning = Translator.get('glitch_gen1_great');

    int statusS = params.statusType == 2 ? 25 : (params.statusType == 1 ? 12 : 0);
    double ballFactor = params.ballId == 'poke' ? 256.0 : (params.ballId == 'great' ? 201.0 : 151.0);
    double hpDivisor = params.ballId == 'great' ? 8.0 : 12.0;

    double hpFactor = min(255.0, (100.0 / max(1.0, params.hpPercent)) * (1020.0 / hpDivisor));

    double p1 = statusS / ballFactor;
    double p2 = min(params.pokemon.captureRate + 1.0, ballFactor - statusS) / ballFactor;
...
class Gen2Strategy extends CatchRateStrategy {
  ...
  CatchRateResult calculate(CatchRateParams params) {
    ...
    String? glitchWarning;
    if (params.ballId == 'love') glitchWarning = Translator.get('glitch_gen2_love');
    if (params.ballId == 'moon') glitchWarning = Translator.get('glitch_gen2_moon');
    if (params.ballId == 'fast') glitchWarning = Translator.get('glitch_gen2_fast');
    ...
    switch (params.ballId) {
      ...
      case 'ultra':
        ballBonus = 2.0; break;
      case 'fast':
        if ([81, 82, 88, 89, 114].contains(params.pokemon.id)) ballBonus = 4.0; break;
      case 'love':
        ballBonus = params.isLoveConditionMet ? 8.0 : 1.0; break;
      case 'moon':
        ballBonus = 1.0; break;
      case 'lure':
        if (params.isFishing) ballBonus = 3.0; break;
      case 'level':
    ...
    double x = max(1.0, ((300.0 - 2.0 * params.hpPercent) * modifiedBaseRate * ballBonus) / 300.0) + sBonus;
